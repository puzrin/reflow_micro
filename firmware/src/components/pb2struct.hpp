...
template <typename T>
bool pb2struct(const std::vector<uint8_t>& pb_data, T& obj, const pb_msgdesc_t* fields_descriptor) {
    pb_istream_t stream = pb_istream_from_buffer(pb_data.data(), pb_data.size());
    return pb_decode(&stream, fields_descriptor, &obj);
}

template <typename T>
bool struct2pb(const T& obj, std::vector<uint8_t>& pb_data, const pb_msgdesc_t* fields_descriptor, size_t max_pb_data_size = 0) {
    if (max_pb_data_size) {
